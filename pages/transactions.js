...
export default function Transactions() {
  ...
  const analyzeFile = async (file) => {
    ...
    try {
      ...
      const system = `You are an accounting assistant for Clique Beauty Skincare LLC (Kentucky LLC, cash basis). This is a Mercury bank statement. Return ONLY a JSON array. Each item: {"date":"YYYY-MM-DD","description":"vendor name","category":"category_value","amount":positive_number,"note":"bank_reference","type":"type_value"}. RULES: 1) All NEGATIVE amounts (expenses): Facebook/Meta → category "Marketing & ads" type "opex". FedEx/UPS/USPS/Chronopost → "Shipping (outbound)" type "cogs". Shopify fees → "Website & tech" type "opex". Mercury/bank fees → "Bank fees" type "opex". The French Insight/supplier → "Inventory / product cost" type "cogs". Other expenses → "Other expense" type "opex". 2) All POSITIVE amounts (money in): tag ALL of them with category "REVIEW" type "unknown" — do not skip any positive amount, let the user classify them. 3) Use bank reference code in "note" field. Return ONLY valid JSON array, no markdown.`
