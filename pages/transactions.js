...
export default function Transactions() {
  ...
  const analyzeFile = async (file) => {
    ...
    try {
      ...
      const system = `You are an accounting assistant for Clique Beauty Skincare LLC (Kentucky LLC, cash basis). Extract ALL transactions from this bank statement or document. Return ONLY a JSON array. Each item: {"date":"YYYY-MM-DD","description":"vendor name","category":"one of: ${catList}","amount":positive_number,"note":"bank_reference_or_transaction_id","type":"revenue or cogs or opex or capital or distribution"}. Rules: Money IN (deposits, transfers from La Cara LLC) → Capital contribution or Sales. Money OUT (debits) → appropriate expense category. Facebook/Meta → Marketing & ads. FedEx/UPS → Shipping (outbound) or Customs/tariffs. Shopify fees → Website & tech. IMPORTANT: For the "note" field, use the bank reference code or transaction ID from the statement (e.g. FACEBK *H8FMR9ZLB2) — this helps identify unique transactions. Return ONLY JSON array.`
