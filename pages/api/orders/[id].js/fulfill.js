...
export default async function handler(req, res) {
  ...
  const pendingLines = lines.filter(
    (l) => l.quantity_fulfilled < l.quantity_ordered
  );

  if (pendingLines.length === 0) {
    return res.status(400).json({ error: "All lines already fulfilled" });
  }
  ...
  // ── 3. For each pending line: update qty_fulfilled + stock ─────────────────
  const now = new Date().toISOString();

  for (const line of pendingLines) {
    const qtyToFulfill = line.quantity_ordered - line.quantity_fulfilled;

    // a) get current WACOG for COGS snapshot
    ...
    // c) update stock_levels: release committed, reduce on_hand permanently
    if (warehouse_id) {
      ...
      // on "confirm" we already reduced qty_on_hand and increased qty_committed
      // on "fulfill" we just release the committed qty (on_hand was already reduced)
      await supabase
        .from("stock_levels")
        .upsert(
          {
            ...
            warehouse_id,
            qty_on_hand:     stock?.qty_on_hand  ?? 0,          // unchanged — already deducted at allocation
