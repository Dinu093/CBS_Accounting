...
export default async function handler(req, res) {
  ...
  // ── 3. Release committed stock for each line ───────────────────────────────
  // Only if status was "confirmed" (allocation was already recorded)
  if (order.status === "confirmed" && warehouse_id) {
    for (const line of lines) {
      ...
      // reverse the allocation: give back qty_on_hand, release committed
      await supabase
        .from("stock_levels")
        .upsert(
          {
      ...
      await supabase
        ...
        .insert({
          ...
          quantity:          pendingQty,           // positive = stock coming back
